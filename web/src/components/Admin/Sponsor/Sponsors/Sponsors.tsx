import { Link, routes } from '@redwoodjs/router'
import { useMutation } from '@redwoodjs/web'
import { toast } from '@redwoodjs/web/toast'

import { QUERY } from 'src/components/Admin/Sponsor/SponsorsCell'
import { truncate } from 'src/lib/formatters'

import type { DeleteSponsorMutationVariables, FindSponsors } from 'types/graphql'

const DELETE_SPONSOR_MUTATION = gql`
  mutation DeleteSponsorMutation($id: Int!) {
    deleteSponsor(id: $id) {
      id
    }
  }
`

const SponsorsList = ({ sponsors }: FindSponsors) => {
  const [deleteSponsor] = useMutation(DELETE_SPONSOR_MUTATION, {
    onCompleted: () => {
      toast.success('Sponsor deleted')
    },
    onError: (error) => {
      toast.error(error.message)
    },
    // This refetches the query on the list page. Read more about other ways to
    // update the cache over here:
    // https://www.apollographql.com/docs/react/data/mutations/#making-all-other-cache-updates
    refetchQueries: [{ query: QUERY }],
    awaitRefetchQueries: true,
  })

  const onDeleteClick = (id: DeleteSponsorMutationVariables['id']) => {
    if (confirm('Are you sure you want to delete sponsor ' + id + '?')) {
      deleteSponsor({ variables: { id } })
    }
  }

  return (
    <div className="rw-segment rw-table-wrapper-responsive">
      <table className="rw-table">
        <thead>
          <tr>
            <th>Id</th>
            <th>Name</th>
            <th>Url</th>
            <th>Logo</th>
            <th>&nbsp;</th>
          </tr>
        </thead>
        <tbody>
          {sponsors.map((sponsor) => (
            <tr key={sponsor.id}>
              <td>{truncate(sponsor.id)}</td>
              <td>{truncate(sponsor.name)}</td>
              <td>{truncate(sponsor.url)}</td>
              <td>{truncate(sponsor.logo)}</td>
              <td>
                <nav className="rw-table-actions">
                  <Link
                    to={routes.adminSponsor({ id: sponsor.id })}
                    title={'Show sponsor ' + sponsor.id + ' detail'}
                    className="rw-button rw-button-small"
                  >
                    Show
                  </Link>
                  <Link
                    to={routes.adminEditSponsor({ id: sponsor.id })}
                    title={'Edit sponsor ' + sponsor.id}
                    className="rw-button rw-button-small rw-button-blue"
                  >
                    Edit
                  </Link>
                  <button
                    type="button"
                    title={'Delete sponsor ' + sponsor.id}
                    className="rw-button rw-button-small rw-button-red"
                    onClick={() => onDeleteClick(sponsor.id)}
                  >
                    Delete
                  </button>
                </nav>
              </td>
            </tr>
          ))}
        </tbody>
      </table>
    </div>
  )
}

export default SponsorsList
